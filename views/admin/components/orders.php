    ...
    <div class="space-y-6">
        <?php foreach ($orders as $order): ?>
            <form method="POST" action="index.php?controllers=admin&action=updateOrderStatus"
                class="grid grid-cols-1 sm:grid-cols-5 gap-4 items-center bg-white border rounded-lg shadow-sm p-5 hover:shadow-md transition js-order-form">
                ...
                <div>
                    ...
                    <p class="text-gray-700">Tổng tiền:
                        <span class="font-semibold text-green-600">
                            <?= number_format((float)($order['payment'] ?? 0), 0, ',', '.') ?> đ
                        </span>
                    </p>
                </div>
