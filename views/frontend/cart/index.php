    ...
    <div>
        ...
        <div class="py-4 border-t border-b text-gray-700 grid grid-cols-[4fr_0.5fr_0.5fr] 
            ...
            <div class="flex items-start gap-6">
                <a href="index.php?controllers=product&action=detail&id=<?= $item['id'] ?>">
                    <img class="w-16 sm:w-20" src="<?= $item['image'] ?>" alt="<?= htmlspecialchars($item['name']) ?>">
                </a>
                <div>
                    <a href="index.php?controllers=product&action=detail&id=<?= $item['id'] ?>"
                        class="text-xs sm:text-lg font-medium"><?= htmlspecialchars($item['name']) ?></a>
...
function updateQuantity(id, size, newQuantity) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $.post('index.php?controllers=Cart&action=loadTotal', function(html) {
                $('#loadTotalCart').html(html);
...
function removeFromCart(id, size, newQuantity) {
